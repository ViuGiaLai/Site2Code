<!-- Source: https://skills.sh/zaggino/z-schema/writing-json-schemas -->
<!-- Installed via: npx skills add zaggino/z-schema@writing-json-schemas -->
---
...
Set `additionalProperties: false` explicitly when extra properties should be rejected â€” z-schema allows them by default.
...
Conditional validation â€” prefer over complex `oneOf` when the logic is "if X then require Y":
...
Prefer `if`/`then`/`else` over `oneOf` when the condition is a single discriminator field â€” it produces clearer error messages.
...
When combining schemas with `allOf`, `additionalProperties: false` in a sub-schema blocks properties defined in sibling schemas. Use `unevaluatedProperties` instead â€” it tracks all properties evaluated across applicators:
