<!-- Source: https://skills.sh/planetscale/database-skills/mysql -->
<!-- Installed via: npx skills add planetscale/database-skills@mysql -->
---
name: mysql
...
- Audit via `performance_schema` â€” drop indexes with `count_read = 0`.
...
## Partitioning
- Partition time-series (>50M rows) or large tables (>100M rows). Plan early â€” retrofit = full rebuild.
...
## Query Optimization
- Check `EXPLAIN` â€” red flags: `type: ALL`, `Using filesort`, `Using temporary`.
- Cursor pagination, not `OFFSET`. Avoid functions on indexed columns in `WHERE`.
- Batch inserts (500â€“5000 rows). `UNION ALL` over `UNION` when dedup unnecessary.
...
- Tune connection pooling â€” avoid `max_connections` exhaustion under load.
