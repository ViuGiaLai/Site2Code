<!-- Source: https://skills.sh/mongodb/agent-skills/mongodb-schema-design -->
<!-- Installed via: npx skills add mongodb/agent-skills@mongodb-schema-design -->
---
...
Data modeling patterns and anti-patterns for MongoDB, maintained by MongoDB. Bad schema is the root cause of most MongoDB performance and cost issuesâ€”queries and indexes cannot fix a fundamentally wrong model.
...
Ask the user:
- What's the primary workload for these collections â€” read-heavy (analytics, reports, searches), write-heavy (logging, IoT ingestion, frequent updates), or balanced?
...
1. Query Stats â†’ identify frequent access patterns (which queries run most often)
2. Slow Query Logs â†’ identify performance bottlenecks (which queries are slow)
...
- **16MB document limit**: MongoDB documents cannot exceed 16MBâ€”this is a hard limit, not a guideline. Common causes: unbounded arrays, large embedded binaries, deeply nested objects. Mitigate by moving unbounded data to separate collections and monitoring document sizes with `$bsonSize`.
...
**âš ï¸ Security**: Use `--readOnly` for safety. Remove only if you need write operations.
...
We're a teamâ€”let's get this right together.
