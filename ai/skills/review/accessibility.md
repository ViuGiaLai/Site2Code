...
<!-- Installed via: npx skills add addyosmani/web-quality-skills@accessibility -->
---
name: accessibility
...
<!-- âœ… Complex image with longer description -->
<figure>
...
| Large text (â‰¥ 18px / â‰¥ 14px bold) | 3:1 | 4.5:1 |
| UI components & graphics | 3:1 | 3:1 |

```css
/* âŒ Low contrast (2.5:1) */
.low-contrast {
  color: #999;
  background: #fff;
}

/* âœ… Sufficient contrast (7:1) */
.high-contrast {
  color: #333;
  background: #fff;
}

/* âœ… Focus states need contrast too (3:1 against background, WCAG 1.4.11) */
:focus-visible {
...
```html
<!-- âŒ Only color indicates error -->
...
**All functionality must be keyboard accessible.** Prefer native interactive elements â€” `<button>`, `<a href>`, and form controls handle Enter/Space activation, focus, and assistive-tech semantics for free. Only add manual keyboard handling when you cannot use a native element.

```html
<!-- âŒ Non-interactive element with click only: not focusable, no keyboard activation -->
<div class="card" onclick="handleAction()">Open</div>

<!-- âœ… Best: use a native button -->
<button type="button" onclick="handleAction()">Open</button>
```

```javascript
// âœ… When you MUST use a non-interactive element (e.g. div with role="button"),
// make it focusable AND handle keyboard activation. Do NOT add this to a native
// <button> â€” Enter/Space already fire click, so you'd double-trigger.
...
**No keyboard traps.** Users must be able to Tab into and out of every component. Use the [modal focus trap pattern](references/A11Y-PATTERNS.md#modal-focus-trap) for dialogsâ€”the native `<dialog>` element handles this automatically.

### Focus visible (2.4.7)

```css
/* âŒ Never remove focus outlines */
*:focus { outline: none; }

/* âœ… Use :focus-visible for keyboard-only focus */
:focus {
  outline: none;
}

:focus-visible {
  outline: 2px solid currentColor; /* inherits text color â†’ already contrast-checked */
  outline-offset: 2px;
}

/* âœ… Or pick a brand color and verify â‰¥3:1 contrast against every background it lands on */
button:focus-visible {
  box-shadow: 0 0 0 3px rgba(0, 95, 204, 0.5);
}
```

### Focus not obscured (2.4.11) â€” new in 2.2
...
```css
/* âœ… Account for sticky headers when scrolling to focused elements */
:target {
  scroll-margin-top: 80px;
}

/* âœ… Ensure focused items clear fixed/sticky bars */
:focus {
  scroll-margin-top: 80px;
...
### Target size (2.5.8) â€” new in 2.2

Interactive targets must be at least **24 Ã— 24 CSS pixels** (AA). Exceptions: inline text links, elements where the browser controls the size, and targets where a 24px circle centered on the bounding box does not overlap another target.

```css
/* âœ… Minimum target size */
button,
[role="button"],
...
/* âœ… Comfortable target size (recommended 44Ã—44) */
.touch-target {
  min-width: 44px;
  min-height: 44px;
  display: inline-flex;
...
### Dragging movements (2.5.7) â€” new in 2.2
...
```html
<!-- âŒ No language specified -->
<html>

<!-- âœ… Language specified -->
<html lang="en">

<!-- âœ… Language changes within page -->
...
### Consistent help (3.2.6) â€” new in 2.2
...
### Redundant entry (3.3.7) â€” new in 2.2
...
```html
<!-- âœ… Auto-fill shipping address from billing -->
<fieldset>
...
### Accessible authentication (3.3.8) â€” new in 2.2
...
```html
<!-- âœ… Allow paste in password fields -->
<input type="password" id="password" autocomplete="current-password">

<!-- âœ… Offer passwordless alternatives -->
...
- [ ] **Target size:** Interactive elements meet 24Ã—24px minimum
