...
<!-- Installed via: npx skills add analogjs/angular-skills@angular-component -->
---
...
Create standalone components for Angular v20+. Components are standalone by defaultâ€”do NOT set `standalone: true`.
...
Use the `host` object in `@Component`â€”do NOT use `@HostBinding` or `@HostListener` decorators.
...
Use native control flowâ€”do NOT use `*ngIf`, `*ngFor`, `*ngSwitch`.
