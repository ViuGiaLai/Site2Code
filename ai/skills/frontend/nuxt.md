<!-- Source: https://skills.sh/antfu/skills/nuxt -->
<!-- Installed via: npx skills add antfu/skills@nuxt -->
---
name: nuxt
...
> **Nuxt 4 note:** the default `srcDir` is `app/` â€” Vue app code (`app.vue`, `components/`, `composables/`, `pages/`, etc.) lives under `app/`, while `server/`, `shared/`, `public/`, `modules/`, `layers/` and `nuxt.config.ts` stay at the project root. The `~`/`@` aliases now point at `app/`; use `~~`/`@@` for the root.
